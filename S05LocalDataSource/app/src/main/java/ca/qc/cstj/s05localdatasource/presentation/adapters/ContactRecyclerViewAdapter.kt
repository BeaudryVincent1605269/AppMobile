...
class ContactRecyclerViewAdapter(var contacts: List<Contact>) : RecyclerView.Adapter<ContactRecyclerViewAdapter.ViewHolder>() {
    ...
    inner class ViewHolder(view: View): RecyclerView.ViewHolder(view){
        ...
        fun bind(contact: Contact) {
            binding.txvContactName.text = contact.name
            binding.txvContactFirstName.text = contact.firstName


            if(contact.isOnline)
            {
                binding.imvCloud.setImageResource(R.drawable.cloud_online)
            } else {
                binding.imvCloud.setImageResource(R.drawable.cloud_offline)
            }
